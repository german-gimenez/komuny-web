...
const tagLabels: Record<string, string> = { fundamental: 'Fundamental', técnico: 'Técnico', práctica: 'Para el aula', ética: 'Ética' };
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.4rem' }}>
            ...
            {term.tag && <span style={{ fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' as const, padding: '2px 8px', borderRadius: '20px', background: color + '18', color, border: `1px solid ${color}40` }}>{tagLabels[term.tag]}</span>}
          </div>
          <p style={{ color: 'var(--ink-muted)', fontSize: '0.92rem', lineHeight: 1.6 }}>{expanded ? term.definition : term.definition.slice(0, 120) + (term.definition.length > 120 ? '…' : '')}</p>
        </div>
        <motion.div animate={{ rotate: expanded ? 180 : 0 }} transition={{ duration: 0.2 }} style={{ color: 'var(--ink-muted)', flexShrink: 0, marginTop: '0.2rem' }}><ChevronDown size={16} /></motion.div>
      </div>
      <AnimatePresence>
        {expanded && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} style={{ overflow: 'hidden' }}>
            <div style={{ paddingTop: '1rem', borderTop: '1px solid var(--border)', marginTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              ...
              {term.seeAlso && <p style={{ fontSize: '0.82rem', color: 'var(--ink-muted)' }}>→ Ver también: <em>{term.seeAlso}</em></p>}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export default function Home() {
  ...
  const grouped = letters.reduce((acc, l) => { const ts = filtered.filter(t => t.letter === l); if (ts.length > 0) acc[l] = ts; return acc; }, {} as Record<string, GlossaryTerm[]>);
  return (
    <main>
      {/* NAV */}
      ...
      <section style={{ minHeight: '88vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem 6rem', position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} style={{ fontSize: '1.15rem', color: 'var(--ink-muted)', maxWidth: '560px', marginBottom: '2.5rem', lineHeight: 1.7 }}>
          Recursos prácticos, abiertos y colaborativos para educadores que quieren integrar inteligencia artificial en el aula.
        </motion.p>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.3 }} style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
          <a href="#glosario" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--ink)', color: 'var(--bg)', padding: '0.75rem 1.75rem', borderRadius: '30px', textDecoration: 'none', fontWeight: 500 }}>Explorar glosario <ArrowRight size={16} /></a>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} style={{ display: 'flex', gap: '3rem', marginTop: '5rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          {[{ n: `${glossaryTerms.length}+`, label: 'Términos en glosario' }, { n: 'LATAM', label: 'Enfoque regional' }, { n: '100%', label: 'Open Source' }].map(s => (
      ...
      <section id="recursos" style={{ padding: '5rem 2rem', background: 'var(--bg-warm)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem' }}>
            {[
              { icon: <BookOpen size={24} />, title: 'Glosario', desc: 'Términos de IA explicados sin tecnicismos, pensados para educadores.', color: '#D4622A', bg: '#FBE9DF', status: '✓ Disponible' },
              { icon: <Cpu size={24} />, title: 'Skills para Claude', desc: 'Configuraciones especializadas listas para usar en el aula.', color: '#3A6B4A', bg: '#E8F2EC', status: '🔜 Próximamente' },
              { icon: <FileText size={24} />, title: 'Templates de Prompts', desc: 'Tickets listos para planear clases, evaluar y comunicarte.', color: '#1A5C9A', bg: '#E0EDF7', status: '🔜 Próximamente' },
              { icon: <GraduationCap size={24} />, title: 'Guías paso a paso', desc: 'Desde tu primera clase con IA hasta proyectos de 4 semanas.', color: '#8B2FC9', bg: '#F2E8FB', status: '🔜 Próximamente' },
            ].map(f => (
      ...
      <section id="glosario" style={{ padding: '5rem 2rem' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div style={{ marginBottom: '3rem' }}>
            ...
            <p style={{ color: 'var(--ink-muted)' }}>{glossaryTerms.length} términos explicados para educadores, sin tecnicismos innecesarios.</p>
          </div>
          <div style={{ marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ position: 'relative' }}>
              <Search size={16} style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--ink-muted)' }} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar término…" style={{ width: '100%', padding: '0.75rem 1rem 0.75rem 2.75rem', background: 'var(--bg-warm)', border: '1.5px solid var(--border)', borderRadius: '12px', fontSize: '0.95rem', color: 'var(--ink)', outline: 'none', fontFamily: 'DM Sans, sans-serif' }} />
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              ...
              {Object.entries(tagColors).map(([tag, color]) => (
                <button key={tag} onClick={() => setActiveTag(activeTag === tag ? null : tag)} style={{ padding: '0.3rem 0.9rem', borderRadius: '20px', fontSize: '0.82rem', cursor: 'pointer', fontWeight: 500, background: activeTag === tag ? color : 'transparent', color: activeTag === tag ? 'white' : color, border: `1.5px solid ${color}60` }}>
                  {{ fundamental: 'Fundamental', técnico: 'Técnico', práctica: 'Para el aula', ética: 'Ética' }[tag]}
                </button>
              ))}
            </div>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--ink-muted)', marginBottom: '1.5rem' }}>{filtered.length} término{filtered.length !== 1 ? 's' : ''} encontrado{filtered.length !== 1 ? 's' : ''}</p>
          ...
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', color: 'var(--ink-muted)' }}>
              <p style={{ fontSize: '1.1rem', marginBottom: '0.5rem' }}>No se encontraron términos</p>
              <p style={{ fontSize: '0.9rem' }}>¿Falta un término? <a href="https://github.com/german-gimenez/komuny/issues/new" style={{ color: 'var(--accent)' }}>Proponlo en GitHub</a></p>
            </div>
          )}
        </div>
      </section>
      ...
      <section style={{ padding: '5rem 2rem', background: 'var(--ink)', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <p style={{ color: 'rgba(245,240,232,0.65)', fontSize: '1.05rem', marginBottom: '2rem', lineHeight: 1.7 }}>Komuny Edu crece con la comunidad. Agrega términos, comparte tus prompts, traduce contenido o cuenta tu experiencia.</p>
      ...
      <footer style={{ padding: '2rem', textAlign: 'center', borderTop: '1px solid var(--border)', fontSize: '0.85rem', color: 'var(--ink-muted)' }}>
        <p>Komuny Edu — Hecho con ❤️ para docentes de América Latina · <a href="https://napsix.ai" style={{ color: 'var(--accent)', textDecoration: 'none' }}>Napsix.AI</a></p>
      </footer>
      <style>{`
